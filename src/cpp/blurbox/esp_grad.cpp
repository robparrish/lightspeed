...
std::shared_ptr<Tensor> BlurBox::espGradSR3(
    ...
    )
{
    ...
    dim.push_back(xyzc->shape()[0]);
    dim.push_back(3);
        
    // The working register of GA
    std::shared_ptr<Tensor> GAT = GA;
    if (!GA) {
        GAT = std::shared_ptr<Tensor>(new Tensor(dim));
    }
    GAT->shape_error(dim);
