...
std::shared_ptr<Tensor> Tensor::chain(
    ...
    double beta)
{
    ...
    std::shared_ptr<Tensor> L = As[0];
    std::shared_ptr<Tensor> R = As[1];
    bool transL = trans[0];
    bool transR = trans[1];
    ...
    LRdim.push_back(nL);
    LRdim.push_back(nR);
    std::shared_ptr<Tensor> T(new Tensor(LRdim));
    C_DGEMM(
        (transL ? 'T' : 'N'),
        (transR ? 'T' : 'N'),
        nL, nR, nK, 1.0,
    ...
    for (size_t i = 2; i < As.size(); i++) {
        ...
        LR2dim.push_back(nL);
        LR2dim.push_back(nR);
        T = std::shared_ptr<Tensor>(new Tensor(LR2dim));
        C_DGEMM(
            (transL ? 'T' : 'N'),
            (transR ? 'T' : 'N'),
            nL, nR, nK, 1.0,
