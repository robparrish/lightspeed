...
std::vector< std::shared_ptr<Tensor> > IntBox::dipoleCPU(
    ...
    )
{
    ...
    std::vector< std::shared_ptr<Tensor> > D2;
    if (Dipole.empty()) {
        for (int d = 0; d < 3; d++) {
            D2.push_back(
    ...
    for (int ang_ind = 0; ang_ind < pairlist->pairlists().size(); ang_ind++) {
        ...
        // number of GH nodes (+1 for dipole)
        ...
        for (size_t ind12 = 0; ind12 < pairsL.size(); ind12++) {
            ...
            for (int j2 = 0; j2 <= i2; j2++) {
                ...
                // X
                D2val[0] = K12 * D1D[0][l1 * V2 + l2] * 
                                 S1D[1][m1 * V2 + m2] * 
                                 S1D[2][n1 * V2 + n2];
                // Y
                D2val[1] = K12 * S1D[0][l1 * V2 + l2] * 
                                 D1D[1][m1 * V2 + m2] * 
                                 S1D[2][n1 * V2 + n2];
                // Z
                D2val[2] = K12 * S1D[0][l1 * V2 + l2] * 
                                 S1D[1][m1 * V2 + m2] * 
                                 D1D[2][n1 * V2 + n2];

                for (int d = 0; d < 3; d++) {
                    #pragma omp atomic
                    D2p[d][(ind1 + off1) * ncart2 + (ind2 + off2)] += D2val[d];
                }
                if (symm && (K1 != K2)) {
                    for (int d = 0; d < 3; d++) {
                        #pragma omp atomic
                        D2p[d][(ind2 + off2) * ncart2 + (ind1 + off1)] += D2val[d];
                    }
                }
    ...
    std::vector< std::shared_ptr<Tensor> > D3;
    if (Dipole.empty()) {
        for (int d = 0; d < 3; d++) {
            D3.push_back(
