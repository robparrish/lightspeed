...
""" 
dftd3.py implements a python version of Grimme's DFTD3 package. Only E(2) in
vacuum boundary conditions is implemented. Analytical gradients are
implemented.

Class Hierarchy:

DFTD3Base - common intermediates
DFTD3(DFTD3Base) - original zero damping recipe
DFTD3BJ(DFTD3Base) - Becke-Johnson damping recipe
DFTD3M(DFTD3) - re-parametrized zero damping recipe with extra "beta" parameter
DFTD3MBJ(DFTD3BJ) - re-parametrized Becke-Johnson damping recipe

Example Usage:

# Quantum chemist's water
Zs = [8, 1, 1]
xyz = np.array([[0.0, 0.0, 0.0], [2.0, 0.0, 0.0], [0.0, 2.0, 0.0]])
# DFT-D3 Usage
d3 = DFTD3Base.build(name='b3-lyp', flavor='bj')
print d3
E = d3.compute_energy(Zs, xyz)
G = d3.compute_gradient(Zs, xyz)
print E
print G

Key papers:

[DFTD3 Paper] - S. Grimme, J. Antony, S. Ehrlich, and H. Krieg, JCP, 132, 154104 (2010).
[DFTD3(BJ) Paper] - S. Grimme, S. Ehrlich, and L. Goerigk, JCC, 32, 1456 (2011).
[DFTD3M/DFTD3M(BJ) Paper] - D.G.A. Smith, L.A. Burns, K. Patkowski, and C.D. Sherrill, JPCL, 7, 2197 (2016).

Units: 
    
All quantities are in au unless otherwise noted.
"""

class DFTD3Base(object):

    """ Class DFTD3Base contains common intermediates for DFTD3 methods. Of
    particular relevance to users are the following:
    ...
    @property
    def C6AB(self):
        """ Dictionary of tabulated C6AB values, used for CNA/CNB-dependent
        interpolation.
        
        Maps (ZA, ZB) keys to [(CNA, CNB, C6)] values, without ZA/ZB
        restrictions. These are extracted from pars.f in Grimme's dftd3
        program, and are stored as an upper-triangle dictionary in
        dftd3_data.py

        See Eg. 16 of the DFTD3 paper.
        """
        if hasattr(self, '_C6AB'): return self._C6AB
        from . import dftd3_data # Huge file, prevents needless import
        # Expand the table from triangular form
        self._C6AB = {}
        for key, val in dftd3_data.C6AB.items():
            self._C6AB[(key[0], key[1])] = val
            if key[0] != key[1]:
                self._C6AB[(key[1], key[0])] = [(y,x,z) for x,y,z in val] # Make sure to reverse CN
        return self._C6AB

    @property
    def k1(self):
        """ (float) k1 exponent in Eq. 15 of DFTD3 paper, used for CN. """ 
        return 16.0
    
    @property
    def k2(self):
        """ (float) k2 exponent in Eq. 15 of DFTD3 paper, used for CN. """
        return 4.0 / 3.0

    @property
    def k3(self):
        """ (float) k3 exponent in Eq. 15 of DFTD3 paper, used for C6AB interpolation. """
        return 4.0

    @property
    def Qsqrt(self):
        """ [np.ndarray, (95,)] Square-roots of QA factors of e.g., Eq. 9 of DFTD3
        paper, used for C8AB recursion and R0AB in BJ. """
        if hasattr(self, '_Qsqrt'): return self._Qsqrt
        self._Qsqrt = np.array([
        0.0, # Gh
    ...
    @property
    def Rcov(self):
        """ RA^cov covalent radii used in computing coordination numbers. See,
        e.g., Eq. 15 of the D3 paper. These already have k2 = 4.0 / 3.0
        multiplied in. """
        if hasattr(self, '_Rcov'): return self._Rcov
        self._Rcov = np.array([
        0.0, # Gh
    ...
    def compute_R0AB(
        ...
        """ Compute the R0AB values for elements Zs (depends on DFTD3 flavor
        via self.R0AB). 
    
        Params:
            Zs (list of int) - list of atomic numbers for the molecule
    
        Returns:
            R0AB [np.ndarray (natom,)*2] - R0AB cutoff distances
        """

        R0AB = np.zeros((len(Zs),)*2)
        for Yind, Y in enumerate(Zs):
            for Zind, Z in enumerate(Zs):
                R0AB[Yind, Zind] = self.R0AB[Y,Z]
        return R0AB
    ...
    def compute_RAB(
        ...
        """ Compute the RAB distance matrix for positions xyz.

        Params:
            xyz [np.ndarray (natom, 3)] - positions of atoms
        
        Return:
            RAB [np.ndarray (natom,)*2] - RAB distances
        """

        x1, x2 = np.meshgrid(xyz[:,0], xyz[:,0], indexing='ij')
        y1, y2 = np.meshgrid(xyz[:,1], xyz[:,1], indexing='ij')
        z1, z2 = np.meshgrid(xyz[:,2], xyz[:,2], indexing='ij')
        RAB = np.sqrt((x1 - x2)**2 + (y1 - y2)**2 + (z1 - z2)**2)
        return RAB
    ...
    def compute_CN(
        ...
        """ Compute the CN atomic coordination numbers.

        Params:
            Zs (list of int) - list of atomic numbers for the molecule
            xyz [np.ndarray (natom, 3)] - positions of atoms
        Return:
            CN [np.ndarray (natom,)] - CN coordination numbers

        See Eq. 15 of the DFTD3 paper.
        """

        RA = np.array([self.Rcov[Z] for Z in Zs])
        with np.errstate(divide='ignore'):
            G = - self.k1 * ((np.outer(RA, np.ones_like(RA)) + np.outer(np.ones_like(RA), RA)) / rAB - 1.0)
        V = 1.0 / (1.0 + np.exp(G))
    ...
    def compute_Qsqrt(
        ...
        """ Extract the sqrt(QA) values for givn Zs.

        Params:
            Zs (list of int) - list of atomic numbers for the molecule
        Return:
            Qsqrt [np.ndarray (natom,)] - sqrt(QA) values
        """

        return np.array([self.Qsqrt[Z] for Z in Zs])
    ...
    def compute_C6AB(
        ...
        """ Compute the C6AB interpolated C6 coefficients

        Params:
            Zs (list of int) - list of atomic numbers for the molecule
            CN [np.ndarray (natom,)] - CN coordination numbers
        Return:
            C6 [np.ndarray (natom,)*2] - C6AB dispersion coefficients

        See Eq. 16 of the DFTD3 paper.
        """

        C6 = np.zeros((len(Zs),)*2)
        for A, ZA in enumerate(Zs):
            CNA = CN[A]
            if ZA == 0: continue # Gh
            for B, ZB in enumerate(Zs):
                CNB = CN[B]
                if A > B: continue # Only do upper triangle
                if ZB == 0: continue # Gh
                C6table = self.C6AB[(ZA,ZB)]
                Z = 0.0
                W = 0.0
                for CNA0, CNB0, C60 in C6table:
                    L = math.exp(-self.k3 * ((CNA - CNA0)**2 + ((CNB - CNB0)**2)))
                    Z += L * C60
                    W += L
                C6[A,B] = C6[B,A] = Z / W
        return C6
    ...
    def compute_C8AB(
        ...
        """ Compute the C8AB interpolated/recursed C8 coefficients

        Params:
            C6 [np.ndarray (natom,)*2] - C6AB dispersion coefficients
            Qsqrt [np.ndarray (natom,)] - sqrt(QA) values
        Return:
            C8 [np.ndarray (natom,)*2] - C8AB dispersion coefficients

        See Eq. 6 of the DFTD3 paper.
        """

        return 3.0 * np.outer(Qsqrt, Qsqrt) * C6
    ...
    def compute_energy_partition(
        ...
        """ Compute the EAB atom-pairwise energy decomposition.

        Params:
            Zs (list of int) - list of atomic numbers for the molecule
            xyz [np.ndarray (natom, 3)] - positions of atoms
        Return:
            EAB [np.ndarray (natom,)*2] - Symmetric matrix with zero diagonal
            with EAB dispersion energy partition. sum(EAB) is the total
            dispersion energy.
        """

        R0AB = self.compute_R0AB(Zs)
        RAB = self.compute_RAB(xyz) 
        CN = self.compute_CN(Zs, RAB)
        Qsqrt = self.compute_Qsqrt(Zs)
        C6AB = self.compute_C6AB(Zs, CN)
        C8AB = self.compute_C8AB(C6AB, Qsqrt)
        EAB = self.compute_EAB(RAB, R0AB, C6AB, C8AB)
        return EAB
    ...
    def compute_energy(
        ...
        """ Compute the total DFTD3 dispersion energy.

        Params:
            Zs (list of int) - list of atomic numbers for the molecule
            xyz [np.ndarray (natom, 3)] - positions of atoms
        Return:
            E (float) - total dispersion energy
        """
        return np.sum(self.compute_energy_partition(Zs, xyz))
    ...
    def compute_gradient(
        ...
        """ Compute the total DFTD3 dispersion energy gradient.

        Params:
            Zs (list of int) - list of atomic numbers for the molecule
            xyz [np.ndarray (natom, 3)] - positions of atoms
        Return:
            G [np.ndarray (natom, 3)] - total dispersion energy gradient
        """
        ...
        C6AB = self.compute_C6AB(Zs, CN)
        C8AB = self.compute_C8AB(C6AB, Qsqrt)
        # Reverse accumulation robomatic differentiation
        dEdRAB, dEdC6AB, dEdC8AB = self.compute_G(RAB, R0AB, C6AB, C8AB)
        dEdC6AB += self.compute_dC8ABdC6AB(dEdC8AB, Qsqrt)
        dEdCN = self.compute_dC6ABdCN(dEdC6AB, Zs, CN)
        dEdRAB += self.compute_dCNdRAB(dEdCN, Zs, RAB) 
        dEdxyz = self.compute_dRABdxyz(dEdRAB, xyz)
        return dEdxyz

    # => Gradient terms <= #

    def compute_dC8ABdC6AB(self, dEdC8AB, Qsqrt):
        """ Compute dEdC6AB contribution from dEdC8AB * dC8ABdC6AB """
        return 3.0 * np.outer(Qsqrt, Qsqrt) * dEdC8AB

    def compute_dC6ABdCN(self, dEdC6AB, Zs, CN):
        """ Compute dEdCN contribution from dEdC6AB * dC6ABdCN """
        dEdCN = np.zeros((len(Zs),))
        for A, ZA in enumerate(Zs):
            CNA = CN[A]
            if ZA == 0: continue # Gh
            for B, ZB in enumerate(Zs):
                ...
                if A >= B: continue # Only do upper triangle
                if ZB == 0: continue # Gh
                dE = 2.0 * dEdC6AB[A,B] # Account for upper/lower triangle
                C6table = self.C6AB[(ZA,ZB)]
                Z = 0.0
                W = 0.0
                for CNA0, CNB0, C60 in C6table:
                ...
                for CNA0, CNB0, C60 in C6table:
                    dEdL = dE * (C60 * W - Z) / W**2
                    L = math.exp(-self.k3 * ((CNA - CNA0)**2 + ((CNB - CNB0)**2)))
                    dEdCN[A] -= 2.0 * self.k3 * (CNA - CNA0) * L * dEdL
                    dEdCN[B] -= 2.0 * self.k3 * (CNB - CNB0) * L * dEdL
        return dEdCN

    def compute_dCNdRAB(self, dEdCN, Zs, RAB):
        """ Compute dEdRAB contribution from dEdCN * dCNdRAB """
        RA = np.array([self.Rcov[Z] for Z in Zs])
        R0AB = np.outer(RA, np.ones_like(RA)) + np.outer(np.ones_like(RA), RA)
        with np.errstate(divide='ignore', invalid='ignore'):
            G = - self.k1 * (R0AB / RAB - 1.0)
            W = np.exp(G)
            V = - W / (1.0 + W)**2 * self.k1 * R0AB / RAB**2
            np.fill_diagonal(V, 0.0)
            dEdRAB = np.outer(dEdCN, np.ones(V.shape[1])) * V
        return dEdRAB

    def compute_dRABdxyz(self, dEdRAB, xyz):
        """ Compute dEdxyz = dEdRAB * dRABdxyz """
        x1, x2 = np.meshgrid(xyz[:,0], xyz[:,0], indexing='ij')
        y1, y2 = np.meshgrid(xyz[:,1], xyz[:,1], indexing='ij')
        z1, z2 = np.meshgrid(xyz[:,2], xyz[:,2], indexing='ij')
        RAB = np.sqrt((x1 - x2)**2 + (y1 - y2)**2 + (z1 - z2)**2)
        with np.errstate(divide='ignore'):
            RABinv = 1.0 / RAB
        np.fill_diagonal(RABinv, 0.0)
        G = np.zeros_like(xyz)
        dEdx12 = dEdRAB * (x1 - x2) * RABinv
        G[:,0] -= np.sum(dEdx12, 0)
        G[:,0] += np.sum(dEdx12, 1)
        dEdy12 = dEdRAB * (y1 - y2) * RABinv
        G[:,1] -= np.sum(dEdy12, 0)
        G[:,1] += np.sum(dEdy12, 1)
        dEdz12 = dEdRAB * (z1 - z2) * RABinv
        G[:,2] -= np.sum(dEdz12, 0)
        G[:,2] += np.sum(dEdz12, 1)
        return G
    ...
    def build(
        ...
        """ Factory constructor for all DFTD3 methods. 

        Params:
            name (str) - the name of the DFT method, e.g., 'b3-lyp'
            flavor (str) - the flavor of the DFTD3 method, one of 'zero', 'zerom', 'bj', or 'bjm'
        Returns:
            d3 (subclass of DFTD3Base) - the properly specialized and initialized DFTD3-type object
        """

        if flavor == 'zero': return DFTD3.build(name)
        elif flavor == 'zerom' : return DFTD3M.build(name)
        elif flavor == 'bj' : return DFTD3BJ.build(name)
        elif flavor == 'bjm' : return DFTD3MBJ.build(name)
        else: raise ValueError('Unknown flavor: %s. Must be one of zero, zerom, bj, bjm' % flavor)

class DFTD3(DFTD3Base):
        
    """ Specialization to -D3 """
    ...
    def __init__(
        ...
        Parameters:
            name (str) - name of the DFT this DFTD3 is built for.
            s6 (float) - coefficient of r^-6 dispersion term. Usually 1.0,
                unless double-hybrid.
            s8 (float) - coefficient of r^-8 dispersion term. A DFT-specific
                parameter in DFTD3.
            sr6 (float) - cutoff scale of r^-6 damping term. A DFT-specific
                parameter in DFTD3.
        Fixed Parameters:
            sr8 (float) - cutoff scale of r^-6 damping term. Fixed to 1.0 in
                DFTD3.
            beta (float) - DFTD3M extra damping parameter. Fixed to 0.0 in DFTD3.
        """
    ...
    def __str__(self):
        """ Short title string of DFTD3 object """
        s = '-D3:\n'
        s += '  %-4s = %10s\n' % ('name', self.name)
        s += '  %-4s = %10.6f\n' % ('s6', self.s6)
        s += '  %-4s = %10.6f\n' % ('s8', self.s8)
    ...
    """ DFT-D3 Zero-Damping Parameters (def2-QZVP). Case 3 in dftd3.f """
    params = {
        "b-lyp" : { 'sr6' : 1.094, 's8' : 1.682 },
        "b-p" : { 'sr6' : 1.139, 's8' : 1.683 },
        "b97-d" : { 'sr6' : 0.892, 's8' : 0.909 },
        "revpbe" : { 'sr6' : 0.923, 's8' : 1.010 },
        "pbe" : { 'sr6' : 1.217, 's8' : 0.722 },
        "pbesol" : { 'sr6' : 1.345, 's8' : 0.612 }, 
        "rpw86-pbe" : { 'sr6' : 1.224, 's8' : 0.901 },
        "rpbe" : { 'sr6' : 0.872, 's8' : 0.514 },
        "tpss" : { 'sr6' : 1.166, 's8' : 1.105 }, 
        "b3-lyp" : { 'sr6' : 1.261, 's8' : 1.703 },
        "pbe0" : { 'sr6' : 1.287, 's8' : 0.928 },
        "hse06" : { 'sr6' : 1.129, 's8' : 0.109 },
        "revpbe38" : { 'sr6' : 1.021, 's8' : 0.862 },
        "pw6b95" : { 'sr6' : 1.532, 's8' : 0.862 },
        "tpss0" : { 'sr6' : 1.252, 's8' : 1.242 },
        "hf" : { 'sr6' : 1.158, 's8' : 1.746 },
        "mpwlyp" : { 'sr6' : 1.239, 's8' : 1.098 },
        "bpbe" : { 'sr6' : 1.087, 's8' : 2.033 },
        "bh-lyp" : { 'sr6' : 1.370, 's8' : 1.442 },
        "tpssh" : { 'sr6' : 1.223, 's8' : 1.219 },
        "pwb6k" : { 'sr6' : 1.660, 's8' : 0.550 },
        "b1b95" : { 'sr6' : 1.613, 's8' : 1.868 },
        "bop" : { 'sr6' : 0.929, 's8' : 1.975 },
        "o-lyp" : { 'sr6' : 0.806, 's8' : 1.764 },
        "o-pbe" : { 'sr6' : 0.837, 's8' : 2.055 },
        "ssb" : { 'sr6' : 1.215, 's8' : 0.663 },
        "revssb" : { 'sr6' : 1.221, 's8' : 0.560 },
        "otpss" : { 'sr6' : 1.128, 's8' : 1.494 },
        "b3pw91" : { 'sr6' : 1.176, 's8' : 1.775 },
        "revpbe0" : { 'sr6' : 0.949, 's8' : 0.792 },
        "pbe38" : { 'sr6' : 1.333, 's8' : 0.998 },
        "mpw1b95" : { 'sr6' : 1.605, 's8' : 1.118 },
        "mpwb1k" : { 'sr6' : 1.671, 's8' : 1.061 },
        "bmk" : { 'sr6' : 1.931, 's8' : 2.168 },
        "cam-b3lyp" : { 'sr6' : 1.378, 's8' : 1.217 },
        "lc-wpbe" : { 'sr6' : 1.355, 's8' : 1.279 },
        "m05" : { 'sr6' : 1.373, 's8' : 0.595 },
        "m052x" : { 'sr6' : 1.417, 's8' : 0.000 },
        "m06l" : { 'sr6' : 1.581, 's8' : 0.000 },
        "m06" : { 'sr6' : 1.325, 's8' : 0.000 },
        "m062x" : { 'sr6' : 1.619, 's8' : 0.000 },
        "m06hf" : { 'sr6' : 1.446, 's8' : 0.000 },
        "hcth120" : { 'sr6' : 1.221, 's8' : 1.206 },
        "b2-plyp" : { 'sr6' : 1.427, 's8' : 1.022, 's6' : 0.64 },
        "pwpb95" : { 'sr6' : 1.557, 's8' : 0.705, 's6' : 0.82 },
        "b2gp-plyp" : { 'sr6' : 1.586, 's8' : 0.760, 's6' : 0.56 },
        "ptpss" : { 'sr6' : 1.541, 's8' : 0.879, 's6' : 0.75 },
    }
    ...
    def build(name):
        ...
        Params: 
            name (str) - DFT method to construct DFTD3 for (must be in DFTD3.params)
        Returns:
            d3 (DFTD3) - the initialized DFTD3 object
        """
        
        if name not in DFTD3.params:
            raise ValueError('Unknown DFTD3 functional: %s' % name)
        pars = DFTD3.params[name]
        return DFTD3(name=name, **pars)

    @property
    def a6(self):
        """ The r^6 damping exponent. """
        return 14.0

    @property
    def a8(self):
        """ The r^8 damping exponent. """
        return 16.0

    @property
    def R0AB(self):
        """ Tabulated R0AB cutoff radii (Eq. 4 in -D3 paper) """
        if hasattr(self, '_R0AB'): return self._R0AB
        # These are in Ang, converted to au below via DFTDFTD3 conversion factor
        Rtemp = np.array([
        ...
        max_elem = 94 
        self._R0AB = np.zeros((max_elem+1,)*2)
        k = 0
        for i in range(1, max_elem+1):
            for j in range(1, i + 1):
                self._R0AB[i,j] = self._R0AB[j,i] = Rtemp[k]
                k += 1
        self._R0AB /= 0.52917726 # Grimme's DFTDFTD3 conversion factor
        return self._R0AB
    ...
    def compute_EAB(
        ...
        """ Specialization of compute_EAB for -D3 and D3M. See Eq. 4 of -D3 and Eq. 2 of -D3M. """

        with np.errstate(divide='ignore', invalid='ignore'):
            E = - 0.5 * self.s6 * C6AB / (RAB**6 * (1.0 + 6.0 * (RAB / (self.sr6 * R0AB) + self.beta * R0AB)**(-self.a6))) \
                - 0.5 * self.s8 * C8AB / (RAB**8 * (1.0 + 6.0 * (RAB / (self.sr8 * R0AB) + self.beta * R0AB)**(-self.a8)))
    ...
    def compute_G(
        ...
        """ Specialization of compute_G for -D3 and D3M. See Eq. 4 of -D3 and Eq. 2 of -D3M. """

        with np.errstate(divide='ignore', invalid='ignore'):
            D6 = RAB**6 * (1.0 + 6.0 * (RAB / (self.sr6 * R0AB) + self.beta * R0AB)**(-self.a6)) 
            D8 = RAB**8 * (1.0 + 6.0 * (RAB / (self.sr8 * R0AB) + self.beta * R0AB)**(-self.a8))
            dD6dRAB = 6.0 * RAB**5 * (1.0 + 6.0 * (RAB / (self.sr6 * R0AB) + self.beta * R0AB)**(-self.a6)) \
                + RAB**6 * (6.0 * -self.a6 * (RAB / (self.sr6 * R0AB) + self.beta * R0AB)**(-self.a6 - 1.0)) / (self.sr6 * R0AB)
            dD8dRAB = 8.0 * RAB**7 * (1.0 + 6.0 * (RAB / (self.sr8 * R0AB) + self.beta * R0AB)**(-self.a8)) \
                + RAB**8 * (6.0 * -self.a8 * (RAB / (self.sr8 * R0AB) + self.beta * R0AB)**(-self.a8 - 1.0)) / (self.sr8 * R0AB)
            dEdC6AB = -0.5 * self.s6 / D6
            dEdC8AB = -0.5 * self.s8 / D8
            dEdRAB = +0.5 * self.s6 * C6AB / D6**2 * dD6dRAB \
                     +0.5 * self.s8 * C8AB / D8**2 * dD8dRAB
        np.fill_diagonal(dEdC6AB, 0.0)
        np.fill_diagonal(dEdC8AB, 0.0)
        np.fill_diagonal(dEdRAB, 0.0)
        return dEdRAB, dEdC6AB, dEdC8AB


class DFTD3M(DFTD3):

    """ Specialization to -D3M """
    ...
    def __init__(
        ...
        Parameters:
            ...
            sr6 (float) - cutoff scale of r^-6 damping term. A DFT-specific
                parameter in DFTD3M.
            beta (float) - DFTD3M extra damping parameter. A DFT-specific
                parameter in DFTD3M.
        Fixed Parameters:
            sr8 (float) - cutoff scale of r^-6 damping term. Fixed to 1.0 in
                DFTD3.
    ...
    """ Modified DFT-DFTD3 Zero-Damping Parameters (Sherrill and co-workers). Case 5 in dftd3.f """
    params = {
        "b2-plyp" : { 'sr6' : 1.313134, 's8' : 0.717543, 'beta' : 0.016035, 's6' : 0.640000 },
        "b3-lyp" : { 'sr6' : 1.338153, 's8' : 1.532981, 'beta' : 0.013988 },
        "b97-d" : { 'sr6' : 1.151808, 's8' : 1.020078, 'beta' : 0.035964 },
        "b-lyp" : { 'sr6' : 1.279637, 's8' : 1.841686, 'beta' : 0.014370 },
        "b-p" : { 'sr6' : 1.233460, 's8' : 1.945174, 'beta' : 0.000000 },
        "pbe" : { 'sr6' : 2.340218, 's8' : 0.000000, 'beta' : 0.129434 },
        "pbe0" : { 'sr6' : 2.077949, 's8' : 0.000081, 'beta' : 0.116755 },
        "lc-wpbe" : { 'sr6' : 1.366361, 's8' : 1.280619, 'beta' : 0.003160 },
    }
    ...
    def build(name):
        ...
        Params: 
            name (str) - DFT method to construct DFTD3M for (must be in DFTD3M.params)
        Returns:
            d3 (DFTD3M) - the initialized DFTD3M object
        """
        if name not in DFTD3M.params:
            raise ValueError('Unknown DFTD3M functional: %s' % name)
        pars = DFTD3M.params[name]
        return DFTD3M(name=name, **pars)

class DFTD3BJ(DFTD3Base):

    """ Specialization to -D3(BJ) """
    ...
    def __init__(
        ...
        """ DFTD3BJ constructor (same as DFTD3MBJ)
        ...
        Parameters:
            ...
            s6 (float) - coefficient of r^-6 dispersion term. Usually 1.0,
                unless double-hybrid.
            s8 (float) - coefficient of r^-8 dispersion term. A DFT-specific
                parameter in DFTD3BJ.
            a1 (float) - constant in BJ damping formula. A DFT-specific
                parameter in DFTD3BJ.
            a2 (float) - constant in BJ damping formula. A DFT-specific
                parameter in DFTD3BJ.
        """
    ...
    """ DFT-DFTD3(BJ) parameters with BJ radii. Case 4 in dftd3.f """
    params = {
        "b-p" : { 'a1' : 0.3946, 's8' : 3.2822, 'a2' : 4.8516 },
        "b-lyp" : { 'a1' : 0.4298, 's8' : 2.6996, 'a2' : 4.2359 },
        "revpbe" : { 'a1' : 0.5238, 's8' : 2.3550, 'a2' : 3.5016 },
        "rpbe" : { 'a1' : 0.1820, 's8' : 0.8318, 'a2' : 4.0094 },
        "b97-d" : { 'a1' : 0.5545, 's8' : 2.2609, 'a2' : 3.2297 },
        "pbe" : { 'a1' : 0.4289, 's8' : 0.7875, 'a2' : 4.4407 },
        "rpw86-pbe" : { 'a1' : 0.4613, 's8' : 1.3845, 'a2' : 4.5062 },
        "b3-lyp" : { 'a1' : 0.3981, 's8' : 1.9889, 'a2' : 4.4211 },
        "tpss" : { 'a1' : 0.4535, 's8' : 1.9435, 'a2' : 4.4752 },
        "hf" : { 'a1' : 0.3385, 's8' : 0.9171, 'a2' : 2.8830 },
        "tpss0" : { 'a1' : 0.3768, 's8' : 1.2576, 'a2' : 4.5865 },
        "pbe0" : { 'a1' : 0.4145, 's8' : 1.2177, 'a2' : 4.8593 },
        "hse06" : { 'a1' : 0.383, 's8' : 2.310, 'a2' : 5.685 }, 
        "revpbe38" : { 'a1' : 0.4309, 's8' : 1.4760, 'a2' : 3.9446 },
        "pw6b95" : { 'a1' : 0.2076, 's8' : 0.7257, 'a2' : 6.3750 },
        "b2-plyp" : { 'a1' : 0.3065, 's8' : 0.9147, 'a2' : 5.0570, 's6' : 0.64 },
        "dsd-blyp" : { 'a1' : 0.0000, 's8' : 0.2130, 'a2' : 6.0519, 's6' : 0.50 },
        "dsd-blyp-fc" : { 'a1' : 0.0009, 's8' : 0.2112, 'a2' : 5.9807, 's6' : 0.50 },
        "bop" : { 'a1' : 0.4870, 's8' : 3.2950, 'a2' : 3.5043 },
        "mpwlyp" : { 'a1' : 0.4831, 's8' : 2.0077, 'a2' : 4.5323 },
        "o-lyp" : { 'a1' : 0.5299, 's8' : 2.6205, 'a2' : 2.8065 },
        "pbesol" : { 'a1' : 0.4466, 's8' : 2.9491, 'a2' : 6.1742 },
        "bpbe" : { 'a1' : 0.4567, 's8' : 4.0728, 'a2' : 4.3908 },
        "opbe" : { 'a1' : 0.5512, 's8' : 3.3816, 'a2' : 2.9444 },
        "ssb" : { 'a1' : -0.0952, 's8' : -0.1744, 'a2' : 5.2170 },
        "revssb" : { 'a1' : 0.4720, 's8' : 0.4389, 'a2' : 4.0986 },
        "otpss" : { 'a1' : 0.4634, 's8' : 2.7495, 'a2' : 4.3153 },
        "b3pw91" : { 'a1' : 0.4312, 's8' : 2.8524, 'a2' : 4.4693 },
        "bh-lyp" : { 'a1' : 0.2793, 's8' : 1.0354, 'a2' : 4.9615 },
        "revpbe0" : { 'a1' : 0.4679, 's8' : 1.7588, 'a2' : 3.7619 },
        "tpssh" : { 'a1' : 0.4529, 's8' : 2.2382, 'a2' : 4.6550 },
        "mpw1b95" : { 'a1' : 0.1955, 's8' : 1.0508, 'a2' : 6.4177 },
        "pwb6k" : { 'a1' : 0.1805, 's8' : 0.9383, 'a2' : 7.7627 },
        "b1b95" : { 'a1' : 0.2092, 's8' : 1.4507, 'a2' : 5.5545 },
        "bmk" : { 'a1' : 0.1940, 's8' : 2.0860, 'a2' : 5.9197 },
        "cam-b3lyp" : { 'a1' : 0.3708, 's8' : 2.0674, 'a2' : 5.4743 },
        "lc-wpbe" : { 'a1' : 0.3919, 's8' : 1.8541, 'a2' : 5.0897 },
        "b2gp-plyp" : { 'a1' : 0.0000, 's8' : 0.2597, 'a2' : 6.3332, 's6' : 0.560 },
        "ptpss" : { 'a1' : 0.0000, 's8' : 0.2804, 'a2' : 6.5745, 's6' : 0.750 },
        "pwpb95" : { 'a1' : 0.0000, 's8' : 0.2904, 'a2' : 7.3141, 's6' : 0.820 },
        # special HF/DFT with eBSSE correction
        "hf/mixed" : { 'a1' : 0.5607, 's8' : 3.9027, 'a2' : 4.5622 },  
        "hf/sv" : { 'a1' : 0.4249, 's8' : 2.1849, 'a2' : 4.2783 },
        "hf/minis" : { 'a1' : 0.1702 , 's8' : 0.9841, 'a2' : 3.8506 },
        "b3-lyp/6-31gd" : { 'a1' : 0.5014 , 's8' : 4.0672, 'a2' : 4.8409 },
        "hcth120" : { 'a1' : 0.3563, 's8' : 1.0821, 'a2' : 4.3359 },
        "dftb3" : { 'a1' : 0.5719, 's8' : 0.5883, 'a2' : 3.6017 },
        "pw1pw" : { 'a1' : 0.3807, 's8' : 2.3363, 'a2' : 5.8844 },
        "pwgga" : { 'a1' : 0.2211, 's8' : 2.6910, 'a2' : 6.7278 },
        "hsesol" : { 'a1' : 0.4650, 's8' : 2.9215, 'a2' : 6.2003 },
        # special HF-DFTD3-gCP-SRB/MINIX parametrization
        "hf3c" : { 'a1' : 0.4171, 's8' : 0.8777, 'a2' : 2.9149 },
        # special HF-DFTD3-gCP-SRB2/ECP-2G parametrization
        "hf3cv" : { 'a1' : 0.3063, 's8' : 0.5022, 'a2' : 3.9856 },
        # special PBEh-DFTD3-gCP/def2-mSVP parametrization
        "pbeh3c" : { 'a1' : 0.4860, 's8' : 0.0000, 'a2' : 4.5000 },
        "pbeh-3c" : { 'a1' : 0.4860, 's8' : 0.0000, 'a2' : 4.5000 },
    }
    ...
    def build(name):
        ...
        Params: 
            name (str) - DFT method to construct DFTD3BJ for (must be in DFTD3BJ.params)
        Returns:
            d3 (DFTD3BJ) - the initialized DFTD3BJ object
        """
        if name not in DFTD3BJ.params:
            raise ValueError('Unknown DFTD3BJ functional: %s' % name)
        pars = DFTD3BJ.params[name]
        return DFTD3BJ(name=name, **pars)

    @property
    def R0AB(self):
        """ Specialization of R0AB = sqrt(C8AB / C6AB) = sqrt(3 * sqrt(QA * QB)) for -D3(BJ) and -D3M(BJ) """
        if hasattr(self, '_R0AB'): return self._R0AB
        self._R0AB = np.sqrt(3.0) * np.sqrt(np.outer(self.Qsqrt, self.Qsqrt))
        return self._R0AB
    ...
    def compute_EAB(
        ...
        """ Specialization of compute_EAB for -D3(BJ) and D3M(BJ). See Eqs. 1 and 4 of -D3(BJ) """

        with np.errstate(divide='ignore', invalid='ignore'):
            E = - 0.5 * self.s6 * C6AB / (RAB**6 + (self.a1 * R0AB + self.a2)**6) \
                - 0.5 * self.s8 * C8AB / (RAB**8 + (self.a1 * R0AB + self.a2)**8)
        np.fill_diagonal(E, 0.0)
        return E
    ...
    def compute_G(
        ...
        """ Specialization of compute_G for -D3(BJ) and D3M(BJ). See Eqs. 1 and 4 of -D3(BJ) """

        with np.errstate(divide='ignore', invalid='ignore'):
            D6 = (RAB**6 + (self.a1 * R0AB + self.a2)**6)
            D8 = (RAB**8 + (self.a1 * R0AB + self.a2)**8)
            dEdC6AB = -0.5 * self.s6 / D6
            dEdC8AB = -0.5 * self.s8 / D8
            dEdRAB = +0.5 * self.s6 * C6AB / D6**2 * 6.0 * RAB**5 \
                     +0.5 * self.s8 * C8AB / D8**2 * 8.0 * RAB**7 
        np.fill_diagonal(dEdC6AB, 0.0)
        np.fill_diagonal(dEdC8AB, 0.0)
        np.fill_diagonal(dEdRAB, 0.0)
        return dEdRAB, dEdC6AB, dEdC8AB

class DFTD3MBJ(DFTD3BJ):

    """ Specialization to -D3M(BJ) """
    ...
    """ Modified DFT-DFTD3(BJ) parameters with BJ radii (Sherrill and co-workers). Case 6 in dftd3.f """
    params = {
        "b2-plyp" : { 'a1' : 0.486434, 's8' : 0.672820, 'a2' : 3.656466, 's6' : 0.640000 },
        "b3-lyp" : { 'a1' : 0.278672, 's8' : 1.466677, 'a2' : 4.606311 },
        "b97-d" : { 'a1' : 0.240184, 's8' : 1.206988, 'a2' : 3.864426 },
        "b-lyp" : { 'a1' : 0.448486, 's8' : 1.875007, 'a2' : 3.610679 },
        "b-p" : { 'a1' : 0.821850, 's8' : 3.140281, 'a2' : 2.728151 },
        "pbe" : { 'a1' : 0.012092, 's8' : 0.358940, 'a2' : 5.938951 },
        "pbe0" : { 'a1' : 0.007912, 's8' : 0.528823, 'a2' : 6.162326 },
        "lc-wpbe" : { 'a1' : 0.563761, 's8' : 0.906564, 'a2' : 3.593680 },
    }
    ...
    def build(name):
        ...
        Params: 
            name (str) - DFT method to construct DFTD3MBJ for (must be in DFTD3MBJ.params)
        Returns:
            d3 (DFTD3MBJ) - the initialized DFTD3MBJ object
        """
        if name not in DFTD3MBJ.params:
            raise ValueError('Unknown DFTD3MBJ functional: %s' % name)
        pars = DFTD3MBJ.params[name]
        return DFTD3MBJ(name=name,**pars)
