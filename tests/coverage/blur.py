...
class RHF(object):
    ...
    def __init__(
        ...
        #print 'SAD Guess:\n'
        #self.tensors['CSAD'] = ls.SAD.compute_Ca(self.minbasisset,self.basisset)
        ...
        #self.tensors['D'] = ls.Tensor.copy(self.tensors['DSAD'])
        self.tensors['D'] = ls.Tensor.zeros_like(self.tensors['H'])
        Eold = 0.
        converged = False
...
def run_h2o_sto3g():
    ...
    rhf = RHF.build(
        ...
        basisname='sto-3g',
        minbasisname='cc-pvdz-minao',
        options={'diis_max_vecs' : 6, 'thre_pq' : 1.0E-12})
    ...
    hash = ls.HashedGrid(grid.xyz,4.0)

    alpha = 0.5
    #alpha = -1.0

    rho = ls.BlurBox.pointDensity(
        res,
        mol.xyzZ,
        hash,
        alpha,
    ...
    print(np.sum(grid.w * rho.np))
        
    grid.save_cube_file('rho.cube', 'rho', mol, rho)

    mol.save_xyz_file('mol.xyz')
